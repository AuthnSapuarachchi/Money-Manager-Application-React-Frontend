...
const Expense = () => {
    ...
    const fetchExpenseCategories = async () => {
        try {
            // Fetch only EXPENSE type categories using the specific endpoint
            const response = await axiosConfig.get(API_ENDPOINTS.categoryByType('EXPENSE'));
            
            if (response.status === 200) {
                const expenseCategories = response.data.categories || response.data;
                console.log('Fetched expense categories:', expenseCategories);
