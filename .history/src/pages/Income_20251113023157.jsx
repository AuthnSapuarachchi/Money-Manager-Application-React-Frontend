...
const Income = () => {
    ...
    const fetchIncomeCategories = async () => {
        try {
            // Fetch only INCOME type categories using the specific endpoint
            const response = await axiosConfig.get(API_ENDPOINTS.categoryByType('INCOME'));
            console.log('Fetched income categories:', response.data);
            
            if (response.status === 200) {
                const incomeCategories = response.data.categories || response.data;
    ...
    const handleEmailIncomeDetails = async () => {
        try{
            ...
            const response = await axiosConfig.post(API_ENDPOINTS.sendIncomeEmail, {
                recipient: toEmail,  // Changed from toEmail to recipient to match backend
                incomes: emailData
            });
            ...
            if (response.status === 200) {
                toast.success(response.data || `Income report sent successfully to ${toEmail}`, {
                    duration: 5000
                });
            }
