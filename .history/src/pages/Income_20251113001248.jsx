...
const Income = () => {
    ...
    const handleDownloadincomeDetails = async() => {
        try{
            ...
            if (!token) {
                toast.error('Please login to download income details');
                return;
            }

            toast.loading('Preparing download...');
            
            const response = await axiosConfig.get(API_ENDPOINTS.incomeExcelDownload, {
                responseType: 'blob',
                headers: {
            ...
            let fileName = `incomes_details_${new Date().toISOString().split('T')[0]}.xlsx`;
            
            // Create blob with explicit type for Excel files
            ...
            if (error.response?.status === 403) {
                ...
                toast.error('Download endpoint not found. Please contact support.');
            } else {
